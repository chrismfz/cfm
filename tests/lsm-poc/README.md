...
| CFML-OBS-004 | `obs-004-ptrace.sh` | Sibling-worker credential theft: watched-uid task forks a victim child (same uid), then `PTRACE_ATTACH`es to it — the canonical post-exploit pattern for stealing in-memory secrets from a running PHP-FPM worker or long-running cron job. Same-uid ptrace is allowed by `kernel.yama.ptrace_scope=1` (default), so any of the user's own processes is a viable target. Event flags carry the ATTACH/READ mode bits and a SAMEUID hint. **Coverage:** full on yama≤1 (the distro default); on yama=2 hosts the BPF LSM hook is pre-empted by yama's earlier -EPERM in the kernel LSM chain — OBS-004 records nothing there, the kernel block IS the protection. To exercise this PoC on a yama=2 host, drop `ptrace_scope` to 1 temporarily. |
| CFML-NET-002 | `net-002-raw-socket.sh` | Scanner / sniffer / spoofing toolkit drop by a compromised vhost user: `socket(AF_INET/INET6, SOCK_RAW, …)` or `socket(AF_PACKET, SOCK_RAW, …)` from a watched uid. The `socket_create` LSM hook fires BEFORE the capability check, so the PoC succeeds even though the calling user has no CAP_NET_RAW (socket(2) returns -EPERM but the event lands first). `ping` handling: SOCK_DGRAM path (open `ping_group_range`) never triggers; SOCK_RAW path with legacy setuid /bin/ping is suppressed by the rule's euid==0 fast-path; SOCK_RAW path with modern cap_net_raw+ep /bin/ping (Debian / Ubuntu / EL9+) WILL fire — NET-002 does not consult the lsm.conf allowlist surface (PolicyRawSocket is not in allowExePolicy, and the filename payload is `proto=NN` not a path), so the only realistic mitigations are to keep the rule in monitor mode and triage manually, or open `net.ipv4.ping_group_range`. Enforce-capable — returns -EPERM out of socket(2). |
| CFML-CRED-004 | `cred-004-cap-raise.sh` | Post-exploit capability hoarding: watched-uid task holds CAP_X in `cap_inheritable` (granted via file caps or a previous commit_creds) and calls `prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_X, …)` to move the cap into the ambient set — where it survives every subsequent `execve()`. The harness pre-stages `setcap cap_net_bind_service+pi` on the helper (root step), then runs it as `TEST_USER`; the helper's prctl call triggers a commit_creds with the new ambient bit, and CRED-004 fires. Complements CFML-CRED-002 (uid→0) and CFML-CRED-003 (direct commit_creds): together they cover the three canonical credential-escalation primitives. Monitor-only — fentry/commit_creds is a trace probe; no kernsec pairing exists. |
