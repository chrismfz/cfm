...
| CFML-NET-002 | `net-002-raw-socket.sh` | Scanner / sniffer / spoofing toolkit drop by a compromised vhost user: `socket(AF_INET/INET6, SOCK_RAW, …)` or `socket(AF_PACKET, SOCK_RAW, …)` from a watched uid. The `socket_create` LSM hook fires BEFORE the capability check, so the PoC succeeds even though the calling user has no CAP_NET_RAW (socket(2) returns -EPERM but the event lands first). Modern unprivileged ping (SOCK_DGRAM + IPPROTO_ICMP) does NOT trigger; setuid /bin/ping runs euid=0 and is filtered by the watched-uid gate. Enforce-capable — returns -EPERM out of socket(2) so the dropper sees the failure and the primitive never lands. |
