...
-- 12xxx panel listeners). Before this module each file carried its own copy
...
--
-- Callers pcall-require this module and keep their previous inline list as a
-- fallback, so an upgrade-lag deploy (new cfm.lua, old /var/lib/cfm/lua set)
-- degrades to the old behaviour instead of failing the request path.
