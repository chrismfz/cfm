...
-- ─────────────────────────────────────────────────────────────────────────────
-- BRIDGE TOKEN  (auto-generated by cfm daemon, written to cfm_bridge_token.lua)
-- ─────────────────────────────────────────────────────────────────────────────
local _BRIDGE_TOKEN_FILE = "/usr/local/openresty/nginx/lua/cfm_bridge_token.lua"
local _bridge_token
do
  local chunk, load_err = loadfile(_BRIDGE_TOKEN_FILE)
  if not chunk then
    ngx.log(ngx.ERR, "[cfm] cannot load bridge token file ", _BRIDGE_TOKEN_FILE,
            ": ", tostring(load_err))
    error("[cfm] missing bridge token file — ensure cfm daemon has started")
  end
  local ok, val = pcall(chunk)
  if not ok or type(val) ~= "string" or #val < 32 then
    ngx.log(ngx.ERR, "[cfm] bridge token file invalid or too short: ", _BRIDGE_TOKEN_FILE)
    error("[cfm] invalid bridge token in " .. _BRIDGE_TOKEN_FILE)
  end
  _bridge_token = val
end
