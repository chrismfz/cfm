...
| `challenge_ip_status` | Dump the DNAT challenge sets (`challenge_v4`/`v6`) with each IP's remaining `ttl_sec`. Debug a "stuck in Checking-your-browser loop" on a DNAT node: still present after solve → release didn't clear it; `ttl_sec` RESETS across polls → engine re-challenges (missing cooldown); counts DOWN → aging out. Empty is normal in edge mode (gate there is the Lua clearance cookie) | `firewall/challenge/list` | — |
...
| 2026.08.11 | challenge-loop debug | **+`challenge_ip_status`** (32 tools) — read-only dump of the DNAT challenge sets (`challenge_v4`/`v6`) with per-IP remaining TTL, to tell a stuck-member (release failed) from a re-added member (engine re-challenges, no cooldown) on a DNAT node |
