...
| `host_access_history` | ARCHIVAL traffic profile of ONE VHOST reconstructed from the edge access log incl. rotated siblings (OpenResty/Angie alike): total requests, per-hour series + peaks-vs-median, status mix, top IPs, top UAs with a browser-envelope vs automation/bot-like split (heuristic UA normalization — `bot_ratio_pct`), top paths, method mix. Reaches far beyond host_drilldown/edge_access_tail retention — but nodes keep limited rotation, so check `coverage_oldest_unix` vs the requested window before claiming "a month". Bounded: shared line budget across files (`max_lines`), one timeout, key-capped tops; corrupt siblings listed in `files_failed`; at most TWO archive scans run per node concurrently (429 + Retry-After beyond); rotated reach ON by default (`include_rotated=false` for live only). `combine` (default on) joins detector history for the same window: challenge issued/solved, block triggers, suspicious, WAF observed + per-rule breakdown; with `merge_www` the twins get a COMBINED view plus `detector_by_host` provenance | `webdet/host-access-history` | `host`*, `hours`, `merge_www`, `include_rotated`, `combine`, `max_files`, `lines`, `max_lines`, `top` |
