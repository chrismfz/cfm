...
47 read-only tools. Each wraps the `/api/v1` endpoint(s) shown (the same the
...
| 2026.08.23 | cfm-lsm detect triage | **+`lsm_detections`** (47 tools) — aggregates the cfm-lsm DETECT lines from CFM's own lsm log (`internal/lsmdetect`, endpoint `/api/v1/system/lsm-detections`): per-policy counts + rate-cap suppression roll-up, top repeat offenders by (policy, comm, exe) with last-seen/user/short-sha256. The "real compromise vs chatty CRED-002 false positive (sssd/sssd_kcm/cagefsctl/panel perl)" triage view that drives `allow_comm=`/`allow_exe=` tuning in `/etc/cfm/lsm.conf`; companion to `cfm_log_tail which=lsm` (raw lines) and `dmesg_tail` (kernel-side copies) |
