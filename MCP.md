...
17 read-only tools. Each wraps the `/api/v1` endpoint(s) shown (the same the
...
| 2026.08.06 | Edge-proxy connectivity fixes (OIDC-discovery alias, DNS-rebinding guard disabled); consent POST rate-limit; enrich reverse-DNS perf (waf/security summaries ~60s→~1.5s) | **+`process_list`, +`listening_ports`** (17 tools) — node system diagnostics (busiest processes; listening sockets + owner) |
