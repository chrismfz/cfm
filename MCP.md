...
| `host_access_history` | ARCHIVAL traffic profile of ONE VHOST reconstructed from the edge's FULL access log (never the focused access.cfm.log) incl. rotated siblings (OpenResty/Angie alike): total requests, per-hour series + peaks-vs-median, status mix, top IPs, top UAs with a browser-envelope vs automation/bot-like split (heuristic UA normalization — `bot_ratio_pct`), top paths, method mix. Malformed/aborted traffic (400/408/414/431/494/499 — kept by the edge OUT of access.log) is scanned separately into `bad_requests.*` with its own totals/status/tops (`nil` = node has no such log); `total_requests_with_bad` is the combined headline. Reaches far beyond host_drilldown/edge_access_tail retention — but nodes keep limited rotation, so check `coverage_oldest_unix` vs the requested window before claiming "a month". Bounded: shared line budget across ALL files of both sections (`max_lines`), one timeout, key-capped tops; corrupt siblings in `files_failed`; EVERY reach bound sets `truncated=true` (incl. `live_tail_truncated`); `log_changed_during_scan=true` flags copytruncate/rotation mid-read (re-run); at most TWO archive scans run per node concurrently (429 + Retry-After beyond); rotated reach ON by default (`include_rotated=false` for live logs only). `combine` (default on) joins detector history over EXACTLY the same absolute window as the access scan: challenge issued/solved, block triggers, suspicious, WAF observed + per-rule breakdown; `detector_coverage` exposes the store's real retention/oldest retained event (`coverage_proven=false` = completeness not provable from retained rows; `detector_partial=true` on a failed twin query; `detector_unavailable=true` when history is unreadable); with `merge_www` the twins get a COMBINED view plus `detector_by_host` provenance | `webdet/host-access-history` | `host`*, `hours`, `merge_www`, `include_rotated`, `combine`, `max_files`, `lines`, `max_lines`, `top` |
