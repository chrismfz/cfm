...
| `detectors_config` | The parsed detectors.conf **with values** — every section's keys (MODE / DRY_RUN / BLOCK / thresholds), the resolved path, `exists`, and `overlay_files`. "What is this knob actually set to?" and "did I leave X in monitor/dryrun instead of block?" — what `config_drift` (presence + stock-vs-live diffs, no live values) and `detectors_status` (runtime activity) can't show. **Base-file only:** overlays under `/etc/cfm/detectors.d/*.conf` are listed by name but their overriding values are NOT merged — cross-check `config_drift`. Admin-only endpoint, already backing the cfm-admin editor | `detectors/config` | — |
...
| 2026.09.18 | detector config values | **+`detectors_config`** (55 tools) — the parsed detectors.conf **with live values** (`/api/v1/detectors/config`, the endpoint already behind the cfm-admin editor): every section's keys map (MODE / DRY_RUN / BLOCK / thresholds), path, `exists`, `overlay_files`. Closes the gap the drift/status/srcresolve trio left open — none returned the actual configured value, so "did HUMANITY_MIN_OBS land at 100?" or "is this family still in dryrun instead of block?" was unanswerable over MCP. Base-file only (overlay values are listed by filename, not merged — cross-check `config_drift`); admin-only read, no new endpoint. (Count corrected 53→55: the header had drifted one behind the registered set before this row.) |
