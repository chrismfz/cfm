...
| Site Cache dirs | `/var/cache/nginx` (traversable, created 0755) with `cfm_static` + `cfm_micro_{1,2,5,10,30,60}s` as `root:cfm 0770`, created and healed by `scripts/cfm-cache-dirs.sh` before `-t` — the same helper the `.deb`/`.rpm` scripts run | same |
