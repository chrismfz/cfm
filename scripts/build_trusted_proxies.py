...
def existing_prefixes(path: str) -> list[str]:
    ...
    for line in lines:
        ...
        if s.startswith(DIRECTIVE + " ") and s.endswith(";"):
            cidr = s[len(DIRECTIVE):-1].strip()
            try:  # skip what isn't a network (unix:, a typo): the rewrite drops it
...
STAMP_PREFIX = "# Generated at: "
ETAG_PREFIX = "# etag: "


def same_content(path: str, data: str) -> bool:
    """Is the file exactly what render() would write now, bar the timestamp
    and the (informational) etag? Then leave it: a release commits nothing."""
    try:
        with open(path, encoding="utf-8") as fh:
            cur = fh.read()
    except OSError:
        return False
    strip = lambda t: [ln for ln in t.splitlines() if not ln.startswith((STAMP_PREFIX, ETAG_PREFIX))]  # noqa: E731
    return strip(cur) == strip(data)
...
def render(v4: list[str], v6: list[str], etag: str) -> str:
    lines = [
        ...
        "# edge's main config.",
        f"{STAMP_PREFIX}{datetime.now(timezone.utc).isoformat()}",
    ]
    if etag:
        lines.append(f"{ETAG_PREFIX}{blp.oneline(etag, 80)}")
    lines.append("")
...
def main() -> int:
    ...
    new = set(v4 + v6)
    old = set(existing)
    if same_content(args.output, render(v4, v6, etag)):
