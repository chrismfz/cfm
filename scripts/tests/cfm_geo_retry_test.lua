...
package.loaded["resty.maxminddb"] = {
  ...
  lookup = function(_) return { country = { iso_code = "US" } } end,
}
...
check(geo.country("1.2.3.4") == "", "first call returns '' when init fails")
...
_now = _now + COOLDOWN + 1
a_fail = false
check(geo.country("1.2.3.4") == "US", "F46: after cooldown a good DB self-heals (country resolved)")
...
check(geo2.country("1.2.3.4") == "", "new_object: first call '' on open failure")
...
_now = _now + COOLDOWN + 1
b_fail = false
check(geo2.country("1.2.3.4") == "GB", "F46: new_object self-heals after cooldown")
