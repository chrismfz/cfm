...
- [x] **[medium]** Proxy real_ip drift: angie.conf uses real_ip_header CF-Connecting-IP/recursive on at http level, while openresty relies on nginx-cfm.conf X-Real-IP/recursive off + nginx-cpanel-server-realip.conf server-level override
  - **Triaged 2026-07-14: NON-ISSUE.** The premise is wrong: the two **edge engines** have byte-identical realip config (`CF-Connecting-IP` / `recursive on` / same Cloudflare trust list). `nginx-cfm.conf` / `nginx-cpanel-server-realip.conf` are NOT the openresty edge — they deploy to the downstream cPanel **origin** (`/etc/nginx`), identical under both engines. The keyed IP (`remote_addr`) stays unforgeable for untrusted peers (F44 invariant). No drift, no spoof.
