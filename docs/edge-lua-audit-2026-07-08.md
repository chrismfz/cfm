...
- [x] **[medium]** configs/lua/log-cfm.lua (155 lines, log_by_lua deferred socket send)
...
- [x] **[medium]** cfm_filecache.lua (109 lines) — the per-worker file-cache module underpinning excludes/token caches
  - **Triaged 2026-07-14: NON-ISSUE.** The hypothesised mtime hazard doesn't apply — freshness is **pure TTL**, not mtime, so a same-second change is picked up when the (short) TTL rolls. Atomic tmp+rename writer (F54) means `loadfile` never sees a torn file (no TOCTOU); failures are negative-cached (no retry storm) and pcall-guarded; token rotation is **actively** invalidated via `refresh_token`/`refresh_token_throttled`, not left to TTL. Nit: `opts.ttl` is documented-required but unenforced (latent footgun; all callers pass it).
...
- [x] **[medium]** cfm-panel-listeners.conf.in (357-line templated panel listener config) + SNI fallback cert path
  - **Triaged 2026-07-14: NON-ISSUE.** Only 2 template vars, both in comment lines (sed-rendered) — no functional directive depends on substitution. Every panel port runs `cfm_panel.lua` with a **trusted** `$cfm_panel_origin`, `proxy_pass $cfm_pass` **fail-closes** to empty, the F41 per-port scope isolation is intact (test-guarded), and the cert fallback is the same safe selfsigned path; a missing fallback can't ship (gated by `nginx -t`). Nit: `$cfm_panel_fail_mode` is set but read by no Lua (dead var; real policy is the `CFM_PANEL_FAIL_OPEN` env, fail-open by design).
...
- [x] **[medium]** TLS/SNI edge: ssl_certificate_by_lua on cache-miss/parse-failure + selfsigned fallback (openresty.conf:992 / angie.conf:974)
  - **Triaged 2026-07-14: NON-ISSUE.** Fail-safe: a cache miss or empty SNI early-returns **before** `ssl.clear_certs()` (called only on a HIT), so the static self-signed default is served — never another tenant's cert. Parse failures are caught at **ingest** (`store_pair` drops the bad entry), so a corrupt cert never reaches the handshake and degrades to the safe fallback. Self-signed key is freshly generated per install (not predictable).
...
- [x] **[medium]** Proxy real_ip drift: angie.conf uses real_ip_header CF-Connecting-IP/recursive on at http level, while openresty relies on nginx-cfm.conf X-Real-IP/recursive off + nginx-cpanel-server-realip.conf server-level override
  - **Triaged 2026-07-14: NON-ISSUE (edge verified; origin-side note).** The two **edge engines** have byte-identical realip: `real_ip_header CF-Connecting-IP; real_ip_recursive on; include trusted_proxies.conf` (openresty.conf:238-240 == angie.conf:242-243). `nginx-cfm.conf` is a **log_format** snippet (README §"Web log format snippets"; deployed to edge *and* origin via `collector.go` hook paths), NOT the edge realip source — so it can't create realip drift on the edge. The keyed client IP (`$remote_addr`) therefore stays unforgeable for untrusted peers on both engines (F44 invariant). Not fully traced: `nginx-cpanel-server-realip.conf`'s server-level override lives on the downstream cPanel **origin** (`/etc/nginx`), outside the edge trust boundary that gates CFM's IP decisions — worth a dedicated origin-side pass if that override ever feeds anything CFM keys on, but it does not affect the edge.
...
- [x] **[low]** init_worker_by_lua bootstrap window (openresty.conf/angie.conf ~102-130) and its +2s retry
  - **Triaged 2026-07-14: NON-ISSUE.** Fails safe — the static self-signed cert holds during the window (the cert miss-branch never clears it), so handshakes complete; the `require`-failure retry schedules **exactly one** successor `ngx.timer.at(2,…)` and stops on success — no spin, no timer accumulation. (Only `sslcollector` bootstraps here; the bridge token loads lazily, so that half of the concern doesn't exist.)
...
- [x] **[low]** cfm_cache_log.lua (71 lines) and cfm_purge purge-all interactions beyond the single force-unblock finding
  - **Triaged 2026-07-14: NON-ISSUE.** There **is** no purge-all path (only per-IP `purge_ip`), and `cfm_cache_log` is a dormant, `if not d`-guarded counter module writing a **separate, undeclared** `cfm_cache_stats` dict — zero shared lock / zero interaction with `purge_ip` (which touches `cfm_decisions`/`cfm_geocache`). Inert in the shipped config.
