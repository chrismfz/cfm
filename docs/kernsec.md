...
| `modules.ipsec` | 1 | `esp4`, `esp6` | Kernel ESP transforms. Mitigates the XFRM/ESP LPE class (CVE-2026-46300 "Fragnesia" and the related "Dirty Frag"). Skipped on hosts with active IPsec policies (host-profile gated on `HasIPsec`). |
...
IPsec/XFRM modules (`esp4`, `esp6`) are shipped in `modules.ipsec` and
blacklisted by default in response to the XFRM/ESP LPE class
(CVE-2026-46300 "Fragnesia" and the related "Dirty Frag"). The rule is
auto-skipped on hosts where the IPsec host-profile gate fires
(`/proc/net/xfrm_policy` or `/proc/net/pfkey` non-empty), so hosts that
actually terminate or transit IPsec tunnels keep the kernel data path.
...
| IPsec | Non-empty `/proc/net/xfrm_policy` or `/proc/net/pfkey`. | Skips the `modules.ipsec` blacklist (`esp4`, `esp6`) so the kernel ESP data path stays available on hosts that actually use it. |
