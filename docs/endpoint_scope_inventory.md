...
- `/api/v1/firewall/challenge/list` via `adminOnlyHandler` (`challenge_list_endpoint.go`) — read-only dump of the DNAT challenge sets (`challenge_v4`/`challenge_v6`) with per-IP remaining TTL, for debugging a "stuck in challenge loop" on a DNAT node; a node-wide challenge set has no per-vhost meaning. Backs the `challenge_ip_status` MCP tool.
