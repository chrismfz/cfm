...
- `/api/v1/challenge/exclude/*`, `/api/v1/waf/exclude/*` (`validateScopedExcludeWrite`: scoped tokens limited to `type=host` within their vhost allowlist; list reads are scope-filtered. WAF add/remove also accept an admin-only `scope_hosts` qualifier via `effectiveExcludeScope`: it can only *narrow* an admin-global entry to specific vhosts — a scoped token's effective scope is always pinned to its own context vhost set, so the param can never widen or redirect a scoped write)
