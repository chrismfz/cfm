...
| 21 | Real-sample audit (2026-05-09). Replayed 4 PHP malware samples captured from hacked WordPress / shared-hosting sites: `forms.php` (in-memory loader, char-table+XOR), `index.php` (eval gadget with $func chains), `index2.php` (WP index spoof, multilingual identifiers, .htaccess writer), `user.php` (hex2bin loader). All four caught at upload time by rules 401+402 (block). At request time samples 2+3 were caught by 404/405; samples 1+4 stalled at score 4 because `include`/`include_once`/`require`/`require_once` weren't in rule 404's scoring callables. Identified gap A (closed in row 22), gap B (request-time encoded-payload detector — deferred), gap C (412 image-MIME scope — by-design), gap D (string-fragment fname detector — defence in depth, deferred), gap E (non-multipart upload path — partially closed by gap A, full closure deferred). | DONE |
| 22 | Rule 404 extension — `include` / `include_once` / `require` / `require_once` added as scoring callables at +1 weight. New helper `has_include_construct` matches both the parens form (`include($f)`) AND the bare-statement form (`include $f;` — what loader-style malware actually uses). New tags `RAW_INCLUDE` / `RAW_INCLUDE_ONCE` / `RAW_REQUIRE` / `RAW_REQUIRE_ONCE`. The +1 weight (vs eval's +3) plus the existing `<?php` (+2) + superglobal (+2) + `;<?php` (+1) signals means the loader pattern reaches score 6 → fires; a bare `<?php include __DIR__."/wp-blog-header.php";` (no superglobal) stays at 4, below threshold. Closes the request-time MISS for sample 1 (`forms.php`) and sample 4 (`user.php`) from the row-21 audit. | DONE |
