...
| **Bots stay off filter / facet URLs** (`bots_no_qs`) | one `block` (360+) or `throttle hard_bot` (170+) per bot group, `has_qs` + `qs_not_rx` = `BOT_QS_PASSTHROUGH` (passes only when EVERY parameter is a click id / UTM / pagination / feed / lang key — `?filter_color=red&page=3` is still blocked), **off** | ladyfox.gr `?min_price&filter_color` grid crawled by Meta; bet-prognostika `?lg-min/lv-max` permutations by Meta + GPTBot; SemrushBot on `?ind=k&ind=n…`. Generalises `block_meta_qs`. |
| **Lock panel service subdomains** (`lock_panel_subdomains`) | (`allow` office IPs 15 on the service list, 16 on the browser list) · `challenge` `cpanel.*, webmail.*` outside \<GR, CY\> (252, off) · `block` `cpcalendars.*, cpcontacts.*, webdisk.*, autodiscover.*, autoconfig.*` outside (312, off) | 401 brute-force at 9 rps on `cpcalendars.*`, uniq-path sweeps on `autodiscover.*`/`autoconfig.*`. DAV / autodiscovery clients cannot solve a challenge, hence block. |
