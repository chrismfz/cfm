...
- on Tier B, a **non-session preference cookie** the app reads server-side: the
  ...
  `viewed_cookie_policy` / `cookielawinfo-checkbox-*`, Moove `moove_gdpr_popup`,
  Complianz `cmplz_*`, Age Gate `age_gate` — as do the language / currency
  cookies), so a consenting or age-verified visitor is served uncached; any
...
override on micro, and on micro no request body forwarded, the exact
