...
bot, waived under `CHALLENGE_GOODBOT_EXEMPT` (`v2_waived=<name>` on the solve
line/history; Google-Read-Aloud scores 140 from rotating first-seen IPs, so
the gate verifies inline — bounded, reject path only) — at the EDGE `challenge_v2`
