...
- **A mass unblock no longer runs fail2ban and imunify360 once per IP.** A
  ...
  entries it couldn't parse), the IPs it didn't show are deleted blindly from
  drop and captcha, as before. `cfm.deny` is rewritten once, the feed sets
  are read once, and the block sets are written in one batch (per IP if that
  batch fails). It used to run every step per IP — up to three
