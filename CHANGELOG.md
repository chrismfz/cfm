...
- **ChallengeV2 rejects are now recorded durably, with country / ASN / PTR —
  ...
  humanity gate refused (`result=v2_reject`) left exactly one trace: a log line
  carrying the IP and nothing else. There was no history row, nothing MCP could
  query, and judging whether a rejected client was a farm or a person meant a
  manual lookup per address. Now:
  - every `result=v2_reject` **and** every `result=solved` line ends in
    `cc=GR asn=6799 asn_name="OTEnet S.A." ptr=…` — the client's network
    identity, resolved once at verify through the enricher's cached-or-async
    path, so verify never waits on reverse DNS. A key is omitted when it is not
    resolved (a cold address has no `ptr` on its first solve), never
    fabricated. Note `ptr=` here is reverse DNS; `sig=ptr:` is a pointer-event
    count.
  ...
  - the solved line keeps its free-text ` - (AS6799 OTEnet S.A., Greece)`
    tail byte-for-byte for existing tooling; it is now rendered from the same
    once-resolved fields rather than a second lookup, so it can no longer
    disagree with the new fields or the history row.
  - `payload.ptr` is admin-only on the scoped (cPanel) surface, like `sig`.
    Country and ASN are not stripped — `enrich=1` already gives them to
    scoped callers.
