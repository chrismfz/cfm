...
- **The cfm-web blocklist feeds are now pulled with the node's `AUTH_TOKEN`.**
  ...
  see it. If cfm-web REFUSES the token (unknown, pinned to another IP, usage
  limit reached — note each feed pull now counts against a limited token), the
  node retries once without it so its lists keep refreshing while cfm-web still
  allows the fallback, and `blocklist_feeds` shows `token_rejected` with
  cfm-web's reason: fix that token before the fallback is turned off. A 401/403
  from a cfm-web-looking feed (the API_URL host, or a `/blacklist.txt` /
  `/whitelist.txt` elsewhere) that got no token says why (`no Token sent: …`).
  Upgrade the whole fleet before cfm-web turns the fallback off.
...
- **MCP `blocklist_feeds` + `GET /api/v1/firewall/feeds`** (admin-only): each
  ...
  `failing` total, and `api_origin` / `token_sent` / `token_rejected` — so
...
- **`make release` also refreshes the Cloudflare ranges in
  ...
  - The edge takes the client IP from `CF-Connecting-IP` only when the peer is
    in this file. It was a hand-kept list (its header still said "Example
    only"). It matches Cloudflare's published list today, so nothing changes
    on the nodes now; a future Cloudflare change reaches the next package on
    its own.
  ...
  - Cloudflare only. QUIC.cloud passes a client-supplied `CF-Connecting-IP`
    through, so trusting it needs its own design.
- **The challenge server no longer keeps its own copy of the Cloudflare
  ranges.** It trusted the client-IP headers from a Cloudflare peer as well as
  from the local edge. Cloudflare never connects to it (it listens on
  `127.0.0.1:9098`, loopback on all 11 nodes), so the copy only risked
  drifting from `trusted_proxies.conf`. Only a loopback or private peer (the
  edge) may now name the client.
- **`make release` refreshes the crawler/CDN bypass list before packaging.**
  - `configs/challenge_waf_bypass.conf` (Google, Bing, DuckDuckGo, OpenAI,
    QUIC.cloud, ... ranges that skip challenge + WAF) was only regenerated by
  ...
  - What does block: the offline validator failing, or no Python >= 3.9 on the
    ...
    release commit leaves the list out.
- **The bypass list now also covers Applebot** (Siri / Spotlight crawler).
...
- **A changed `NFT_DNAT_PRIORITY` now takes effect on a cfm restart.**
  ...
  `cfm.conf` sets when it differs (a brief rebuild, as any `cfm dnat on`), keeping its current ports and re-reading the bypass file,
  and logs `re-applied: NFT_DNAT_PRIORITY X (was Y)`. A value nft refuses for a
  nat chain (-200 or below) keeps the installed chain, with a log line, and
  `cfm dnat on` now refuses such a priority before touching the live table (the
  nft backend deleted it first, so a refused add left web DNAT off). It leaves the
