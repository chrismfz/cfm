...
- **Solver-farm detection now catches the low-and-slow farm** via a
  ...
  single-country). Alert-only/shadow like the parent track, and fail-safe OFF
  without GeoIP. New `[challenge_solver_farm]` keys: `FP_TRACK` (default on),
  `MIN_FP_SUBNETS` (8), `MIN_FP_COUNTRIES` (6), `ALLOW_FPS`. Calibrated on a live
  fleet capture — see `docs/solver-farm-fingerprint-concentration.md`.
