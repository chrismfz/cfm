...
func WriteClamavLuaConfig(luaPath string, enabled bool, cfmGID int) error {
	...
	tmp := luaPath + ".tmp"
	if err := os.WriteFile(tmp, []byte(content), 0640); err != nil { // #nosec G306 -- group-read intentional: cfm group = nginx workers only
		return fmt.Errorf("clam: write clamav lua config tmp: %w", err)
	}
	if err := os.Chmod(tmp, 0640); err != nil {
		logging.Logf("[clam] WARNING: failed chmod on tmp clamav lua config %s: %v", tmp, err)
	}
	if cfmGID > 0 {
		if err := os.Chown(tmp, 0, cfmGID); err != nil {
			logging.Logf("[clam] WARNING: failed chown on tmp clamav lua config %s to root:%d: %v", tmp, cfmGID, err)
		}
	}
	if err := os.Rename(tmp, luaPath); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("clam: rename clamav lua config: %w", err)
	}
	if cfmGID > 0 {
		_ = os.Chown(luaPath, 0, cfmGID)
	}
