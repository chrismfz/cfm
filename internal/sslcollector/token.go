...
func WriteClamavLuaConfig(luaPath string, enabled bool, cfmGID int) error {
	...
	tmp := luaPath + ".tmp"
	if err := os.WriteFile(tmp, []byte(content), 0640); err != nil { // #nosec G306 -- group-read intentional: cfm group = nginx workers only
		return fmt.Errorf("sslcollector: write clamav lua config tmp: %w", err)
	}
	if err := os.Chmod(tmp, 0640); err != nil {
		logging.Logf("[sslcollector] WARNING: failed chmod on tmp clamav lua config %s: %v", tmp, err)
	}
	if cfmGID > 0 {
		if err := os.Chown(tmp, 0, cfmGID); err != nil {
			logging.Logf("[sslcollector] WARNING: failed chown on tmp clamav lua config %s to root:%d: %v", tmp, cfmGID, err)
		}
	}
	if err := os.Rename(tmp, luaPath); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("sslcollector: rename clamav lua config: %w", err)
	}
	if cfmGID > 0 {
		_ = os.Chown(luaPath, 0, cfmGID)
	}
...
// so OpenResty workers can read it. cfm.lua / cfm_panel.lua loadfile() this at
// init_worker time to apply per-daemon config without env plumbing.
//
// Currently carried:
//   - clearance_refresh: when true, accepted requests re-mint cfm_clearance
//     with exp = now + cookie_life so active panel/webmail users don't get
//     re-challenged mid-session.
func WriteWebdetectorBridgeConfig(luaPath string, clearanceRefresh bool, cfmGID int) error {
	if !filepath.IsAbs(luaPath) {
		return fmt.Errorf("sslcollector: luaPath must be absolute, got %q", luaPath)
	}
	clearanceRefreshStr := "true"
	if !clearanceRefresh {
		clearanceRefreshStr = "false"
	}
	content := fmt.Sprintf(
		"-- AUTO-GENERATED by CFM \xe2\x80\x94 do not edit\nreturn {\n  clearance_refresh = %s,\n}\n",
		clearanceRefreshStr,
	...
	tmp := luaPath + ".tmp"
	if err := os.WriteFile(tmp, []byte(content), 0640); err != nil { // #nosec G306 -- group-read intentional: cfm group = nginx workers only
		return fmt.Errorf("sslcollector: write webdetector bridge config tmp: %w", err)
	}
	if err := os.Chmod(tmp, 0640); err != nil {
		logging.Logf("[sslcollector] WARNING: failed chmod on tmp bridge config %s: %v", tmp, err)
	}
	if cfmGID > 0 {
		if err := os.Chown(tmp, 0, cfmGID); err != nil {
			logging.Logf("[sslcollector] WARNING: failed chown on tmp bridge config %s to root:%d: %v", tmp, cfmGID, err)
		}
	}
	if err := os.Rename(tmp, luaPath); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("sslcollector: rename webdetector bridge config: %w", err)
	}
	if cfmGID > 0 {
		_ = os.Chown(luaPath, 0, cfmGID)
	}
