...
func TestWriteWebdetectorBridgeConfigContent(t *testing.T) {
	...
	if !strings.Contains(got, "cookie_life_sec = 3600,") {
		t.Fatalf("expected cookie_life_sec = 3600, got: %q", got)
	}
	// Panel modes were left empty on this cfg → must render the fleet default
	// "enforce" (an unset field must not publish a blank the Lua sees as absent).
	if !strings.Contains(got, `panel_waf_mode = "enforce",`) {
		t.Fatalf("expected panel_waf_mode = \"enforce\" (empty→default), got: %q", got)
	}
	...
	// SiteCache unset on this cfg → the writer renders the bool field faithfully
	// as false (it must never publish a blank the Lua could misread as absent).
	// The master default (on) lives in manager.go's kvBool, not the writer.
	...
	if !strings.Contains(got, "fp_policy = false,") {
		t.Fatalf("expected fp_policy = false, got: %q", got)
	}
	// SiteCache set true on the flipped cfg → renders true (master gate on).
	if !strings.Contains(got, "site_cache = true,") {
		t.Fatalf("expected site_cache = true, got: %q", got)
	}
	// MicroCacheEnforce set true → renders true (Tier B enforce armed).
	if !strings.Contains(got, "micro_cache_enforce = true,") {
		t.Fatalf("expected micro_cache_enforce = true, got: %q", got)
	}

	// Panel-mode normalisation: off/logonly pass through; an unknown token
	// (typo, stray value) must resolve to the enforce default, never a blank.
	cfg = WebdetectorBridgeConfig{
		PanelWAFMode:      "off",
	...
	data, err = os.ReadFile(luaPath)
	if err != nil {
		t.Fatalf("read after modes: %v", err)
	}
	got = string(data)
	if !strings.Contains(got, `panel_waf_mode = "off",`) {
		t.Fatalf("expected panel_waf_mode = \"off\", got: %q", got)
	}
	if !strings.Contains(got, `panel_decision_mode = "enforce",`) {
...
func TestWriteClamavLuaConfigContent(t *testing.T) {
	...
	// enabled = true
	if err := WriteClamavLuaConfig(luaPath, true, false, "async", 3000, 0); err != nil {
	...
	if err := WriteClamavLuaConfig(luaPath, false, false, "async", 3000, 0); err != nil {
