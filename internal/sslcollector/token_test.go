...
func TestWriteWebdetectorBridgeConfigContent(t *testing.T) {
	...
	if err := WriteWebdetectorBridgeConfig(luaPath, true, 0); err != nil {
		t.Fatalf("WriteWebdetectorBridgeConfig(true): %v", err)
	}
	data, err := os.ReadFile(luaPath)
	if err != nil {
		t.Fatalf("read: %v", err)
	...
	// clearance_refresh = false (atomic overwrite)
	if err := WriteWebdetectorBridgeConfig(luaPath, false, 0); err != nil {
		t.Fatalf("WriteWebdetectorBridgeConfig(false): %v", err)
	}
	data, err = os.ReadFile(luaPath)
	if err != nil {
