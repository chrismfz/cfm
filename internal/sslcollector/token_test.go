...
func TestWriteWebdetectorBridgeConfigContent(t *testing.T) {
	...
	// SiteCache unset on this cfg → renders false (the caching master gate is
	// OFF by default; it must never publish a blank the Lua could misread).
	if !strings.Contains(got, "site_cache = false,") {
		t.Fatalf("expected site_cache = false (unset→off), got: %q", got)
	}
