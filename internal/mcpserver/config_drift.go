...
func registerConfigDrift(srv *mcp.Server, d Deps) {
	mcp.AddTool(srv, &mcp.Tool{
		...
		Name:        "config_drift",
		Description: "Stock-vs-live CONFIG DRIFT for this node: 'which features did a CFM release ship that my /etc/cfm configs never received?'. Live conffiles are seeded at install and upgrades never touch them, so new sections/keys accumulate in the packaged reference (/usr/share/cfm/configs/) while older hosts stay blind to them. For detectors.conf (parsed with the SAME reader the daemon uses, so commented-out stock sections don't count): missing_sections (e.g. a whole [waf_security] or [challenge_solver_farm] your live file predates), missing_keys (section exists but a newer knob like challenge_cookie_discard's MIN_SOLVES isn't there — usually an INACTIVE feature, though a key whose code default is on, such as [webdetector] SITE_CACHE / MICRO_CACHE_ENFORCE, runs at that default), extra live-only sections/keys, and value_diffs (informational — per-host values legitimately differ). For cfm.conf: stock-documented keys absent from the live text entirely (commented-out counts as seen). Overlays (/etc/cfm/detectors.d/*.conf) count toward missing_sections/missing_keys (a feature adopted via an overlay is active, so it is NOT reported missing) but their values are deliberately not diffed — they are intentional per-host state, summarized separately (detectors_overlays: per-file section/key counts). Workflow: read summary counts → list missing keys/sections → decide per feature whether to enable (put the tuned block in an /etc/cfm/detectors.d/*.conf overlay — keeps the base conffile pristine and package-updateable — which the daemon hot-reloads automatically) — nothing here changes any file.",
