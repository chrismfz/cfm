...
func NewLoader(opts LoaderOptions) (*Loader, error) {
	...
	for _, id := range wanted {
		...
		if len(entries) == 0 {
			...
			if id == PolicyUnexpectedBPF || id == PolicyInterpreterNetStdio || id == PolicyFdCredMismatch || id == PolicyEphemeralExec || id == PolicyPrivInstall || id == PolicyKernelModuleLoad || id == PolicyKernelKnobWrite || id == PolicyKexecLoad || id == PolicyPtraceAccess {
