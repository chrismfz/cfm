...
//	<pinDir>/links/cfm_ephemeral_exec    — CFML-EXEC-006 attached link
...
const (
	...
	pinFileMapKernelKnobInodes    = "cfm_kernel_knob_inodes"
	...
	pinFileLinkEphemeralExec      = "cfm_ephemeral_exec"
	pinFileLinkFs007Setattr       = "cfm_fs007_setattr"
	pinFileLinkFs007Setxattr      = "cfm_fs007_setxattr"
	pinFileLinkExec007Init        = "cfm_exec007_init"
	pinFileLinkExec007Finit       = "cfm_exec007_finit"
	pinFileLinkFs008              = "cfm_fs008"
	pinFileLinkExec008Kexec       = "cfm_exec008_kexec"
	pinFileLinkExec008KexecFile   = "cfm_exec008_kexec_file"
	pinFileLinkObs004             = "cfm_obs004"
	pinFileLinkNet002             = "cfm_net002"
	pinFileLinkCred004            = "cfm_cred004"
)
...
func NewLoader(opts LoaderOptions) (*Loader, error) {
	...
	for _, id := range wanted {
		...
		if len(entries) == 0 {
			...
			if id == PolicyUnexpectedBPF || id == PolicyInterpreterNetStdio || id == PolicyFdCredMismatch || id == PolicyEphemeralExec || id == PolicyPrivInstall || id == PolicyKernelModuleLoad || id == PolicyKernelKnobWrite || id == PolicyKexecLoad || id == PolicyPtraceAccess || id == PolicyRawSocket || id == PolicyCapRaise {
...
func (l *Loader) pinAll(pinDir string) error {
	...
	for _, p := range []struct {
		...
		{l.objs.cfmlsmMaps.CfmKernelKnobInodes, pinFileMapKernelKnobInodes},
	} {
...
func rewriteConstants(spec *ebpf.CollectionSpec, modes map[PolicyID]Mode, fs005WebOriginMonitor bool) error {
	...
	// CFML-EXEC-006 — same source-only pattern as EXEC-004: operators
	// run `make bpf` locally to add cfm_enforce_ephemeral_exec to the
	// embedded object. Until then, keep disabled-policy loads working
	// and refuse to silently load an EXEC-006 with no enforce knob.
	if vs, ok := spec.Variables["cfm_enforce_ephemeral_exec"]; ok {
		if err := vs.Set(enforceByte(modes[PolicyEphemeralExec])); err != nil {
			return fmt.Errorf("set cfm_enforce_ephemeral_exec=%d: %w", enforceByte(modes[PolicyEphemeralExec]), err)
		}
	} else if modes[PolicyEphemeralExec] != ModeDisabled {
		return fmt.Errorf("BPF spec missing variable %q — run `make bpf` before enabling %s", "cfm_enforce_ephemeral_exec", PolicyEphemeralExec)
	}

	// CFML-FS-007 — same source-only pattern. The constant ships in
	// the embedded .o produced by `make bpf` against the FS-007
	// additions; older .o builds tolerate a disabled FS-007 but refuse
	// to silently load the policy without an enforce knob.
	if vs, ok := spec.Variables["cfm_enforce_priv_install"]; ok {
		if err := vs.Set(enforceByte(modes[PolicyPrivInstall])); err != nil {
			return fmt.Errorf("set cfm_enforce_priv_install=%d: %w", enforceByte(modes[PolicyPrivInstall]), err)
		}
	} else if modes[PolicyPrivInstall] != ModeDisabled {
		return fmt.Errorf("BPF spec missing variable %q — run `make bpf` before enabling %s", "cfm_enforce_priv_install", PolicyPrivInstall)
	}

	// CFML-FS-008 — same pattern.
	if vs, ok := spec.Variables["cfm_enforce_kernel_knob_write"]; ok {
		if err := vs.Set(enforceByte(modes[PolicyKernelKnobWrite])); err != nil {
			return fmt.Errorf("set cfm_enforce_kernel_knob_write=%d: %w", enforceByte(modes[PolicyKernelKnobWrite]), err)
		}
	} else if modes[PolicyKernelKnobWrite] != ModeDisabled {
		return fmt.Errorf("BPF spec missing variable %q — run `make bpf` before enabling %s", "cfm_enforce_kernel_knob_write", PolicyKernelKnobWrite)
	}

	// CFML-NET-002 — same pattern.
	if vs, ok := spec.Variables["cfm_enforce_raw_socket"]; ok {
		if err := vs.Set(enforceByte(modes[PolicyRawSocket])); err != nil {
			return fmt.Errorf("set cfm_enforce_raw_socket=%d: %w", enforceByte(modes[PolicyRawSocket]), err)
		}
	} else if modes[PolicyRawSocket] != ModeDisabled {
		return fmt.Errorf("BPF spec missing variable %q — run `make bpf` before enabling %s", "cfm_enforce_raw_socket", PolicyRawSocket)
	}
...
func pinLinkFiles(id PolicyID) []string {
	...
	case PolicyEphemeralExec:
		return []string{pinFileLinkEphemeralExec}
	case PolicyPrivInstall:
		return []string{
			pinFileLinkFs007Setattr,
			pinFileLinkFs007Setxattr,
		}
	case PolicyKernelModuleLoad:
		return []string{
			pinFileLinkExec007Init,
			pinFileLinkExec007Finit,
		}
	case PolicyKernelKnobWrite:
		return []string{pinFileLinkFs008}
	case PolicyKexecLoad:
		return []string{
			pinFileLinkExec008Kexec,
			pinFileLinkExec008KexecFile,
		}
	case PolicyPtraceAccess:
		return []string{pinFileLinkObs004}
	case PolicyRawSocket:
		return []string{pinFileLinkNet002}
	case PolicyCapRaise:
		return []string{pinFileLinkCred004}
	}
	return nil
}
...
func AdoptPinned(pinDir string, opts LoaderOptions) (*Loader, error) {
	...
	for _, p := range []struct {
		...
		{pinFileMapKernelKnobInodes, func(mp *ebpf.Map) { l.objs.cfmlsmMaps.CfmKernelKnobInodes = mp }},
	} {
		path := filepath.Join(mapsDir, p.name)
...
func (l *Loader) programsFor(id PolicyID) []programEntry {
	...
	case PolicyEphemeralExec:
		return compact(lsmProgramEntry(generatedProgramByName(&progs, "cfm_ephemeral_exec"), pinFileLinkEphemeralExec))
	case PolicyPrivInstall:
		return compact(
			lsmProgramEntry(l.pickedDriftProgram("bpf_lsm_inode_setattr_fs007"), pinFileLinkFs007Setattr),
			lsmProgramEntry(l.pickedDriftProgram("bpf_lsm_inode_setxattr_fs007"), pinFileLinkFs007Setxattr),
		)
	case PolicyKernelModuleLoad:
		return compact(
			tracepointProgramEntry(generatedProgramByName(&progs, "cfm_exec007_init"), pinFileLinkExec007Init, "syscalls", "sys_enter_init_module"),
			tracepointProgramEntry(generatedProgramByName(&progs, "cfm_exec007_finit"), pinFileLinkExec007Finit, "syscalls", "sys_enter_finit_module"),
		)
	case PolicyKernelKnobWrite:
	...
	case PolicyRawSocket:
		return compact(lsmProgramEntry(generatedProgramByName(&progs, "cfm_net002"), pinFileLinkNet002))
	case PolicyCapRaise:
		return compact(fentryProgramEntry(generatedProgramByName(&progs, "cfm_cred004"), pinFileLinkCred004, "commit_creds"))
	}
	return nil
}
...
// KernelKnobInodesMap returns the BPF hash map that CFML-FS-008 consults
// for the small set of sensitive /proc/sys and /sys nodes (core_pattern,
// modprobe_path, sysrq-trigger, uevent_helper, binfmt_misc/register).
// Keys are cfm_inode_key (dev + ino); presence means "writes here are
// post-exploit fingerprints unless the writer is in the trusted set."
// Populated by the daemon stat()ing each shipped path at adoption time.
func (l *Loader) KernelKnobInodesMap() *ebpf.Map {
	return l.objs.cfmlsmMaps.CfmKernelKnobInodes
}
