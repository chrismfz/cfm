...
// TestAllPolicies_Scope confirms exactly the seventeen currently-supported
...
func TestAllPolicies_Scope(t *testing.T) {
	...
	if len(policies) != 17 {
		t.Fatalf("policy catalog should have exactly seventeen entries; got %d", len(policies))
	}
	want := map[PolicyID]bool{
		PolicyMemfdExec:           false,
		PolicyReverseShell:        false,
		PolicyDeletedFileExec:     false,
		PolicyInterpreterNetStdio: false,
		PolicySensitiveWrite:      false,
		PolicyCredEscal:           false,
		PolicyDirectCredInstall:   false,
		PolicyUnexpectedBPF:       false,
		PolicyFdCredMismatch:      false,
		PolicyEphemeralExec:       false,
		PolicyPrivInstall:         false,
		PolicyKernelModuleLoad:    false,
		PolicyKernelKnobWrite:     false,
		PolicyKexecLoad:           false,
		PolicyPtraceAccess:        false,
		PolicyRawSocket:           false,
		PolicyCapRaise:            false,
	}
