...
func (m mockEngineDiag) CardinalitySetNames() map[string]string {
	return map[string]string{"block": "block_v4", "allow": "allow_v4", "ignore": "ignore_v4", "feed": "block_ext_v4_hosts"}
}
...
func TestCollectFirewallStatusParityFieldsAcrossEngines(t *testing.T) {
	...
	keys := []string{"block_cardinality", "allow_cardinality", "ignore_cardinality", "feed_cardinality", "throttled_v4", "port_scanners_v4"}
	for _, k := range keys {
	...
	if !nft.Features["dnat_edge"] || !nftlib.Features["dnat_edge"] {
		t.Fatalf("both engines must publish dnat_edge")
	}
	if _, ok := nft.Features["dnat_challenge"]; ok {
		t.Fatalf("retired dnat_challenge feature must not be published")
	}
	if _, ok := nftlib.Features["dnat_challenge"]; ok {
		t.Fatalf("retired dnat_challenge feature must not be published")
	}
}
