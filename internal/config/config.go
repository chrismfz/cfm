...
type APIConfig struct {
	URL       string
	AuthToken string
	// ADMIN_TOKEN_IP_BINDING — source-IP gate on the admin-token auth branch:
	// "off" (default), "logonly" (burn-in: log would-block, still allow), or
	// "enforce" (403 from any source outside loopback ∪ selfIPs ∪ cfm.allow /
	// cfm.dyndns ∪ the API_URL host). Neutralizes a leaked admin token from a
	// foreign IP without touching scoped tokens, embed cookies or sessions.
	AdminTokenIPBinding string
...
func ParseCFMConf(r io.Reader) (*Config, error) {
	...
	for s.Scan() {
		...
		case "ADMIN_TOKEN_IP_BINDING":
			cfg.API.AdminTokenIPBinding = val
		case "MCP_TOKEN":
			cfg.API.MCPToken = val
		case "MCP", "MCP_ENABLED":
			b := parseBool(val)
			cfg.API.MCPEnabled = &b
...
func IsKnownKey(key string) bool {
	...
	switch key {
	case "API_URL", "AUTH_TOKEN", "TOKEN", "ADMIN_TOKEN_IP_BINDING", "MCP_TOKEN", "MCP", "MCP_ENABLED", "AUTOBLOCK_SEND_TO_API", "MANUAL_BLOCK_SEND_TO_API", "UNBLOCK_SEND_TO_API", "DETECTORS_SEND_TO_API",
