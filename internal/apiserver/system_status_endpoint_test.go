...
func TestSystemStatusEndpoints_Authz(t *testing.T) {
	...
	t.Run("scoped token forbidden lsm-status", func(t *testing.T) {
		rr := doSystemStatusReq(h, http.MethodGet, "/api/v1/system/lsm-status", scoped.Token, false)
		if rr.Code != http.StatusForbidden {
			t.Fatalf("status=%d want=%d body=%s", rr.Code, http.StatusForbidden, rr.Body.String())
		}
	})
	...
	// RunStatus reads /proc + /sys (safe on any linux runner); the shape is the
	// CLI --json wire format augmented with effective_allows.
	t.Run("admin token can read lsm-status", func(t *testing.T) {
		rr := doSystemStatusReq(h, http.MethodGet, "/api/v1/system/lsm-status", "admin-secret", false)
		if rr.Code != http.StatusOK {
			t.Fatalf("status=%d want=%d body=%s", rr.Code, http.StatusOK, rr.Body.String())
		}
		var body struct {
			...
			EffectiveAllows []struct {
				Policy    string   `json:"policy"`
				Mode      string   `json:"mode"`
				AllowExe  []string `json:"allow_exe"`
				AllowComm []string `json:"allow_comm"`
				AllowPath []string `json:"allow_path"`
			} `json:"effective_allows"`
		}
		if err := json.Unmarshal(rr.Body.Bytes(), &body); err != nil {
			t.Fatalf("decode response: %v (%s)", err, rr.Body.String())
		}
		if len(body.Policies) == 0 {
			t.Fatalf("expected policies array from status JSON, got %s", rr.Body.String())
		}
		if len(body.EffectiveAllows) != len(body.Policies) {
			t.Fatalf("effective_allows (%d) must cover every policy (%d): %s",
				len(body.EffectiveAllows), len(body.Policies), rr.Body.String())
		}
	})
