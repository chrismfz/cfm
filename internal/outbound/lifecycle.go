...
func (l *Lifecycle) ApplyConfig(ctx context.Context, cfg *cfgpkg.OutboundConfig) {
	...
	go func() {
		...
		logging.Logf("[outbound] sentinel started (group=%d window=%s smtp/min=%d uniq_dst/min=%d http/min=%d)",
			cc.Group, rt.Window, rt.SMTPPerWindow, rt.UniqueDstPerWindow, rt.HTTPPerWindow)
	}()
}
