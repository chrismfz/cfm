...
// Row is a compact summary for CLI/JSON.
type Row struct {
...
func New(cfg Config) *Detector {
	...
	if cfg.UseEnrich {
		if e, _ := enrich.New(cfg.EnrichDirs...); e != nil {
			d.enr = e
			logging.Logf("[detectors][nginx] enrichment enabled (dirs=%v)", cfg.EnrichDirs)
		}
	}
	return d
}
...
func (d *Detector) Name() string { if d.name != "" { return d.name }; return "nginx_access" }
...
func (d *Detector) RunOnce(ctx context.Context, out chan<- core.Alert) error {
	...
	// rotate bucket(s) on tick and evaluate
	if d.agg.RotateIfDue(now) {
		for _, host := range d.agg.Hosts() {
			m := d.agg.Metrics(host) // computed over last WINDOW
			...
			trigger := (m.RPSTotal >= d.cfg.RPSTotalMin) &&
				...
				errBranch &&
				(m.MedianPerIPRPS <= d.cfg.MedianIPRPSMax)

			if d.agg.MarkConsecutive(host, trigger) >= 2 && d.gate.Allow("host:"+host, now, int(m.RPSTotal), int(d.cfg.RPSTotalMin)) {
				// decorate host with enrich top talkers (optional)
...
type LogRec struct {
	TS float64
	IP, Host, Method, URI, Proto string
	Status int
	Bytes  int64
	RT, URT float64
	UA string
...
func parseTSV(line string) (LogRec, bool) {
	// Expect 12 fields (ts, ip, host, method, uri, proto, status, bytes, rt, urt, ref, ua)
...
func (a *hostAggregator) Metrics(host string) Metrics {
	...
	if m.UniqueIPs > 0 {
		// build tiny histogram buckets of per-IP rps
		...
		// derive median bucket
		target := (m.UniqueIPs+1)/2
		sum := 0
		for i, hcnt := range hist {
			sum += hcnt
			if sum >= target {
				// use mid of bucket as approx
				switch i {
