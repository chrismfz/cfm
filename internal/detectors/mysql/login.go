...
func NewMySQL(cfg LoginConfig) *MySQL {
	...
	if !cfg.StrictEmbeddedConfirm { /* leave as is */
	} else {
		cfg.StrictEmbeddedConfirm = true
	}
...
func (m *MySQL) Every() time.Duration {
	if m.cfg.Every > 0 {
		return m.cfg.Every
	}
...
func (m *MySQL) flush(now time.Time, out chan<- core.Alert) {
	for sk, p := range m.pending {
		...
		if !m.gate.Allow(sk, now, n, limit) {
			continue
		}
...
func (m *MySQL) decorateIP(ip string) string {
	...
	var tag []string
	if asn > 0 {
		tag = append(tag, strconv.Itoa(asn))
	}
	if asname != "" {
		tag = append(tag, asname)
	}
	if country != "" {
		tag = append(tag, country)
	}
	parts := []string{ip}
	if ptr != "" {
...
func newestErrFile(dir string) string {
	...
	for _, de := range d {
		...
		name := de.Name()
		if !strings.HasSuffix(strings.ToLower(name), ".err") {
			continue
		}
		fi, err := de.Info()
		if err != nil {
