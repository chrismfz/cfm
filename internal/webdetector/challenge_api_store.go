...
func NewChallengeAPIStore(maxEvents int) *ChallengeAPIStore {
    if maxEvents <= 0 { maxEvents = 50000 }
...
func (s *ChallengeAPIStore) addEvent(e ChallengeEvent) {
    if len(s.events) == 0 { return }
    s.events[s.head] = e
    s.head = (s.head + 1) % len(s.events)
    if s.size < len(s.events) { s.size++ }
}
...
func (s *ChallengeAPIStore) RecordIPChallenge(ip, host, rule, uri, method string, status int, ttl time.Duration) {
    ...
    if uri != "" { st.LastURI = uri }
    if method != "" { st.LastMethod = method }
    if status != 0 { st.LastStatus = status }
...
func (s *ChallengeAPIStore) ListVhosts(status, mode string, limit int) []ChallengeVhostState {
    ...
    for _, v := range s.vhosts {
        if v == nil { continue }
        if status != "" && status != "all" && v.Status != status { continue }
        if mode != "" && mode != "all" && v.Mode != mode { continue }
        out = append(out, *v)
    }
...
func (s *ChallengeAPIStore) GetVhost(host string) (ChallengeVhostState, bool) {
    ...
    defer s.mu.RUnlock()
    v, ok := s.vhosts[host]
    if !ok || v == nil { return ChallengeVhostState{}, false }
    return *v, true
}
...
func (s *ChallengeAPIStore) ListIPs(host, state string, limit int) []ChallengeIPState {
    ...
    for _, v := range s.ips {
        if v == nil { continue }
        if host != "" && v.Host != host { continue }
        if state != "" && state != "all" && v.State != state { continue }
        out = append(out, *v)
    }
...
func (s *ChallengeAPIStore) GetIP(ip string) (ChallengeIPState, bool) {
    ...
    defer s.mu.RUnlock()
    v, ok := s.ips[ip]
    if !ok || v == nil { return ChallengeIPState{}, false }
    return *v, true
}
...
func (s *ChallengeAPIStore) Events(host, ip, rule, typ string, limit int) []ChallengeEvent {
    ...
    for i := 0; i < s.size && len(out) < limit; i++ {
        ...
        e := s.events[idx]
        if e.Ts.IsZero() { continue }
        if host != "" && e.Host != host { continue }
        if ip != "" && e.IP != ip { continue }
        if rule != "" && e.Rule != rule { continue }
