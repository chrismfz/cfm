...
// TestUATop_HybridGate is the contract test for the hybrid lazy gate:
// per-UA request counts are ALWAYS aggregated (so `cfm bots top` works
// on a fresh box), but per-UA unique-IP sets only activate when at
// least one emergency rule is installed (to bound memory under crawler
// load on idle boxes). A future refactor that breaks either side of
// this contract should fail here.
func TestUATop_HybridGate(t *testing.T) {
	e := NewEngine(Config{
		Every:                 5 * time.Second,
		Window:                2 * time.Minute,
		UAEmergencyStorePath:  t.TempDir() + "/r.json",
	...
	now := float64(time.Now().Unix())
	// Ingest 20 events for one UA from 3 distinct IPs with NO rule
	// installed. Per-UA counts MUST aggregate; unique-IP set MUST stay
	// empty (we only see the count, not the cardinality).
	...
	rows := e.UATop(10)
	if len(rows) == 0 {
		t.Fatal("UATop returned 0 rows with no rule installed; per-UA counts should always aggregate")
	}
	var fb *UATopRow
	for i := range rows {
		if rows[i].UA == "facebookexternalhit" {
			fb = &rows[i]
		}
	}
	if fb == nil {
		t.Fatalf("facebookexternalhit row missing: %+v", rows)
	}
	if fb.Reqs != 20 {
		t.Errorf("Reqs = %d, want 20 (request counts must aggregate even without rules)", fb.Reqs)
	}
	if fb.Vhosts != 1 {
		t.Errorf("Vhosts = %d, want 1", fb.Vhosts)
	}
	if fb.UniqueIPs != 0 {
		t.Errorf("UniqueIPs = %d, want 0 (IP-set tracking must be gated off without rules)", fb.UniqueIPs)
	}

	// Install a rule. UniqueIP tracking activates for SUBSEQUENT events.
	// The events ingested above stay UniqueIPs=0 because their buckets
	// never allocated the IP set; new events into the current bucket
	// trigger lazy-init.
	...
	if fb == nil {
		t.Fatalf("facebookexternalhit row missing post-Set: %+v", rows)
	}
	if fb.Reqs != 25 {
		t.Errorf("Reqs = %d, want 25 (all events should count)", fb.Reqs)
	}
	if fb.UniqueIPs == 0 {
		t.Error("UniqueIPs = 0 after Set; expected post-rule events to populate the IP set")
	}
}
