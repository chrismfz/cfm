...
type AnalyzeOptions struct {
	// MaxLines <= 0 means scan the full configured offline source.
	MaxLines int64

	// Last filters matches to the trailing duration present in the scanned log.
	// Last <= 0 preserves the historical unfiltered behavior.
	Last time.Duration
}
...
func (e *Engine) AnalyzeIP(ip string, maxLines int64) (AnalyzeIPResult, error) {
	return e.AnalyzeIPWithOptions(ip, AnalyzeOptions{MaxLines: maxLines})
}

// AnalyzeIPWithOptions scans the offline log for one IP, optionally keeping only
// records inside the trailing duration present in the scanned source.
func (e *Engine) AnalyzeIPWithOptions(ip string, opts AnalyzeOptions) (AnalyzeIPResult, error) {
	res := AnalyzeIPResult{IP: ip}

	cutoff, useCutoff, err := e.offlineCutoff(opts)
	if err != nil {
		return res, err
	}
	...
	err = e.scanOfflineLines(opts.MaxLines, func(line string) bool {
		rec, ok := e.adapter.Parse(line)
		if !ok {
			return true
		}
		if useCutoff && rec.TS < cutoff {
			return true
		}
		if rec.IP != ip {
			return true
		...
		// κρατάμε min/max TS μόνο για τα matched records μετά το filtering.
		if !firstSet {
			res.FirstTS = rec.TS
			res.LastTS = rec.TS
			firstSet = true
...
func (e *Engine) AnalyzeHost(host string, maxLines int64) (AnalyzeHostResult, error) {
	return e.AnalyzeHostWithOptions(host, AnalyzeOptions{MaxLines: maxLines})
}

// AnalyzeHostWithOptions scans the offline log for one host, optionally keeping
// only records inside the trailing duration present in the scanned source.
func (e *Engine) AnalyzeHostWithOptions(host string, opts AnalyzeOptions) (AnalyzeHostResult, error) {
...
// offlineCutoff returns the timestamp cutoff for a trailing offline analyze window.
// The window is anchored to the newest parsed timestamp in the scanned source,
// not wall-clock time, so delayed or replayed logs behave forensically.
func (e *Engine) offlineCutoff(opts AnalyzeOptions) (float64, bool, error) {
	if opts.Last <= 0 {
		return 0, false, nil
	}
	...
	return newest - opts.Last.Seconds(), true, nil
}
