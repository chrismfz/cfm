...
func TestSiteCacheEdgeParityFixture(t *testing.T) {
	...
	if string(want) != got {
		t.Fatalf("%s is stale — the feed or StatsKeyFor changed. Regenerate it (go test ./internal/webdetector -run TestSiteCacheEdgeParityFixture -update-edge-parity), then run make test-lua: the edge must give the same answers.\n--- generated:\n%s", edgeParityFixture, got)
	}
}
