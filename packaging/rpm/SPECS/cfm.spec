...
# subdirs under these cache roots as the worker (now cfm); a tree left
# root-owned by an older global-cache run blocks the cfm worker — "Permission
# denied while reading upstream" aborts the response and static assets break.
# The daemon and installers only chown the TOP dir, so heal the whole tree once
# here. A cheap O(16) probe of the level-1 dirs skips the recursive walk when
# the tree is already cfm-group (the common case on every later upgrade).
if getent group cfm >/dev/null 2>&1; then
