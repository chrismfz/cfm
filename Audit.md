...
- [ ] ChallengeServer consumes the same identity/scheme rules before direct mounting. — **OPEN; one approach attempted and deliberately declined.** `internal/webdetector/challenge_server.go` keeps its own `clientIP()` (CF-Connecting-IP, XFF-first-element, trusts private peers via `isTrustedProxyPeer`) and its own `X-Forwarded-Proto` handling; the edge feeds it `$scheme`, and it is not yet directly mounted in apiserver. This is the natural companion to Step 2 ("Mount `/__cfm_challenge` and `/__cfm_verify` in apiserver").
  - **Decision (2026-08-25) — do NOT unify by fleet-wide loopback-only narrowing.** A loopback-only unification (shared `internal/reqident` package; challenge server adopting the apiserver's loopback-only trust) was implemented, reviewed across three rounds, and then **closed unmerged (PR #1342)**. It was behavior-preserving for the shipped topology but **removed the challenge server's tolerance for any non-loopback front-end** (Cloudflare-direct, private/public load balancers, multi-hop `X-Forwarded-For`); under a non-standard or future off-loopback path, client identity collapses to the proxy IP (mass-block / mis-attribution risk). Judged not worth the fleet risk. **A future challenge direct-mount must solve identity with an explicit per-listener trust policy** (trust rules bound to the specific listener the request arrived on) rather than narrowing the challenge server's existing fleet-wide behavior. The attempt + full analysis live on branch `claude/audit-item8-challenge-identity` (`docs/security/challenge-identity-unification.md`).

> **Status (2026-08-25).** Items 1–7 landed at source via #1338 (canonical request identity / XFF-overwrite / effective scheme) and this branch (item-7 regression test). Verified against `main` at the current head: edge configs (both engines) audited block-by-block; Go identity in `request_identity.go` with `TestRequestPeerEntryTopologies`; login regression in `TestLoginLimiterIgnoresForgedXFFForPerIPBucket`. Item 8 (ChallengeServer identity/scheme unification) remains the one open Step 1 box: a loopback-only unification was attempted and **deliberately declined** (PR #1342 closed unmerged, 2026-08-25 — see the decision note on item 8 above). It is not deferred-and-planned but explicitly re-scoped: any future challenge direct-mount should use an explicit per-listener trust policy, not a fleet-wide narrowing.
