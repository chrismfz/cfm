...
- [x] ChallengeServer consumes the same identity/scheme rules before direct mounting. — the one rule now lives in the shared leaf package `internal/reqident` (loopback-only forwarded trust; fail-closed on ambiguous chains), consumed by **both** the apiserver (`request_identity.go` wraps it) and the challenge server (`clientIP`/`trustedForwardedProto` delegate to it). The retired-DNAT-era broad trust (`isTrustedProxyPeer` private/link-local/Cloudflare, `CF-Connecting-IP`, XFF-first, ungated `X-Forwarded-Proto`) is removed; behavior-preserving for all live (loopback) edge/panel traffic. Also fixed the `cfm_chal` `Secure` inconsistency. Tests: `reqident_test.go`, `challenge_identity_test.go`. Design: `docs/security/challenge-identity-unification.md`. **The actual direct mounting is Step 2** — this box only covers adopting the rule, which was its prerequisite.

> **Status (2026-08-25).** Items 1–7 landed at source via #1338 (canonical request identity / XFF-overwrite / effective scheme) and this branch (item-7 regression test). Verified against `main` at the current head: edge configs (both engines) audited block-by-block; Go identity in `request_identity.go` with `TestRequestPeerEntryTopologies`; login regression in `TestLoginLimiterIgnoresForgedXFFForPerIPBucket`. Item 8 (ChallengeServer identity/scheme unification) landed in a follow-up branch: the shared `internal/reqident` rule is now consumed by both the apiserver and the challenge server (`docs/security/challenge-identity-unification.md`). **All eight Step 1 boxes are now closed.** The remaining challenge work — actually mounting `/__cfm_challenge` + `/__cfm_verify` in the apiserver — is Step 2, for which item 8 was the prerequisite.
