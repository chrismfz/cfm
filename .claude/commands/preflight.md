...
3. `./scripts/tests/check_test_isolation.sh arm` (records CFM's system dirs —
   `/var/lib/cfm`, `/run/cfm`, `/etc/cfm`, `/var/log/cfm`. If one is missing it
   prints the one-time `sudo install -d …`, for a host without CFM. If one
   exists but isn't writable by you it fails without offering to re-own it —
   never do that on a CFM host. CI seeds them like a packaged node first:
   `ci_seed_cfm_dirs.sh`, CI-only)
4. `go test -race ./...`
